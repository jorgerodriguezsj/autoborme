...
class PROVINCIA_EUS:
    ...
    def from_title(title):
        try:
            if title == 'ARABA/ÁLAVA':
                return PROVINCIA_EUS.ALAVA
            title = remove_accents(title).replace(' ', '_')
            return getattr(PROVINCIA_EUS, title)
        except AttributeError:
            raise ValueError('InvalidProvince: %s' % title)

# print(vars(PROVINCIA))
ALL_PROVINCIAS = list(filter(lambda x: not x.startswith('__') and x != 'from_title', vars(PROVINCIA)))
# EUSKADI = list(filter(lambda x: not x.startswith('__') and x != 'from_title', vars(PROVINCIA_EUS)))
# EUSKADI = ['ARABA/ÁLAVA','ALAVA', 'ARABA', 'GUIPUZCOA', 'GIPUZKOA', 'VIZCAYA', 'BIZKAIA']
EUSKADI = ['ARABA/ÁLAVA','GIPUZKOA', 'BIZKAIA']
EUS_ALREDEDORES = ['ARABA/ÁLAVA', 'BURGOS', 'GIPUZKOA', 'LA_RIOJA', 'NAVARRA', 'CANTABRIA', 'BIZKAIA']
